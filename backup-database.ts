...
async function backupTable(tableName: string): Promise<void> {
  try {
    ...
    const errorMessage = `Error backing up table ${tableName}: ${error instanceof Error ? error.message : 'Unknown error'}`;
...
function createBackupSummary(): void {
  ...
  tables.forEach(table => {
    try {
      ...
      if (fs.existsSync(tableFile)) {
        ...
        (summary.tableBreakdown as any)[table] = tableData.recordCount;
      }
    } catch (error) {
