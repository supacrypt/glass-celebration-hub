...
-- Run this directly in Supabase Dashboard SQL Editor with service_role permissions
-- This fixes all 24+ security issues identified by the linter

-- 1. Enable RLS on tables that don't have it (24 tables)
...
-- 2. Create basic policies for public read access (wedding app context)

-- App Settings - Admin only write
CREATE POLICY "app_settings_read" ON public.app_settings FOR SELECT USING (true);
CREATE POLICY "app_settings_admin" ON public.app_settings FOR ALL USING (auth.jwt() ->> 'role' IN ('admin', 'service_role'));

-- Wedding Events - Public read, auth write
...
-- Chat Messages - Auth users only
CREATE POLICY "chat_messages_auth" ON public.chat_messages FOR ALL USING (auth.uid() IS NOT NULL);
...
-- Transportation/Accommodation - Public read
...
-- Content/Gift Registry - Public read
...
-- Social Feed - Auth users
CREATE POLICY "social_feed_auth" ON public.social_feed FOR ALL USING (auth.uid() IS NOT NULL);
CREATE POLICY "social_feed_likes_auth" ON public.social_feed_likes FOR ALL USING (auth.uid() IS NOT NULL);

-- Carpool/Transport booking - Auth users
CREATE POLICY "bus_seat_bookings_auth" ON public.bus_seat_bookings FOR ALL USING (auth.uid() IS NOT NULL);
CREATE POLICY "carpool_coordination_auth" ON public.carpool_coordination FOR ALL USING (auth.uid() IS NOT NULL);
CREATE POLICY "carpool_participants_auth" ON public.carpool_participants FOR ALL USING (auth.uid() IS NOT NULL);
CREATE POLICY "guest_transport_status_auth" ON public.guest_transport_status FOR ALL USING (auth.uid() IS NOT NULL);

-- 3. Fix tables with RLS enabled but no policies (31 tables)

-- Chat Members - User access only
CREATE POLICY "chat_members_user" ON public.chat_members FOR ALL USING (
  auth.uid() IS NOT NULL AND (auth.uid()::text = user_id OR auth.jwt() ->> 'role' IN ('admin', 'service_role'))
);

-- Other essential policies
CREATE POLICY "conversation_settings_auth" ON public.conversation_settings FOR ALL USING (auth.uid() IS NOT NULL);
...
CREATE POLICY "direct_chats_auth" ON public.direct_chats FOR ALL USING (auth.uid() IS NOT NULL);
...
CREATE POLICY "message_likes_auth" ON public.message_likes FOR ALL USING (auth.uid() IS NOT NULL);
CREATE POLICY "message_reactions_auth" ON public.message_reactions FOR ALL USING (auth.uid() IS NOT NULL);
CREATE POLICY "message_replies_auth" ON public.message_replies FOR ALL USING (auth.uid() IS NOT NULL);
CREATE POLICY "message_reports_auth" ON public.message_reports FOR ALL USING (auth.uid() IS NOT NULL);
CREATE POLICY "messages_auth" ON public.messages FOR ALL USING (auth.uid() IS NOT NULL);
...
CREATE POLICY "photo_likes_auth" ON public.photo_likes FOR ALL USING (auth.uid() IS NOT NULL);
...
CREATE POLICY "poll_notifications_auth" ON public.poll_notifications FOR ALL USING (auth.uid() IS NOT NULL);
CREATE POLICY "poll_options_read" ON public.poll_options FOR SELECT USING (true);
CREATE POLICY "poll_votes_auth" ON public.poll_votes FOR ALL USING (auth.uid() IS NOT NULL);
...
CREATE POLICY "post_comments_auth" ON public.post_comments FOR ALL USING (auth.uid() IS NOT NULL);
CREATE POLICY "post_reactions_auth" ON public.post_reactions FOR ALL USING (auth.uid() IS NOT NULL);
CREATE POLICY "profiles_read" ON public.profiles FOR SELECT USING (true);
CREATE POLICY "profiles_write" ON public.profiles FOR ALL USING (
  auth.uid() IS NOT NULL AND (auth.uid() = id OR auth.jwt() ->> 'role' IN ('admin', 'service_role'))
);
...
CREATE POLICY "user_blocks_user" ON public.user_blocks FOR ALL USING (
  auth.uid() IS NOT NULL AND (auth.uid()::text = blocking_user_id OR auth.jwt() ->> 'role' IN ('admin', 'service_role'))
);
CREATE POLICY "user_roles_read" ON public.user_roles FOR SELECT USING (true);
CREATE POLICY "user_roles_admin" ON public.user_roles FOR ALL USING (auth.jwt() ->> 'role' IN ('admin', 'service_role'));
...
-- Run the db lint command again to verify: supabase db lint --linked