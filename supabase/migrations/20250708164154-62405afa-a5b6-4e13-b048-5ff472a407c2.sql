...
-- Phase 1: Enhanced Social Database Schema
...
CREATE TABLE public.post_comments (
    ...
    CONSTRAINT max_comment_depth CHECK (
        parent_id IS NULL OR 
        (SELECT parent_id FROM public.post_comments WHERE id = post_comments.parent_id) IS NULL
    )
);
