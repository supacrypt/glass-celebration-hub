-- Create dietary_requirements table
...
-- Users can manage their own dietary requirements
CREATE POLICY "Users can manage their own dietary requirements" ON public.dietary_requirements
    FOR ALL USING (
        EXISTS (
...
-- Admins can manage all dietary requirements
CREATE POLICY "Admins can manage all dietary requirements" ON public.dietary_requirements
...
-- Create function to sync dietary requirements with rsvps table
...
-- Create trigger to sync dietary requirements
CREATE TRIGGER sync_dietary_requirements_trigger
