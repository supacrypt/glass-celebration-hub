...
-- Migrate existing hardcoded FAQs from the React component
DO $$
DECLARE
...
BEGIN
  ...
  -- Also migrate the content_blocks FAQs that don't duplicate the above
  INSERT INTO faq_items (category_id, question, answer, display_order, is_featured, is_active)
  SELECT 
    CASE 
      WHEN cb.title LIKE '%time%' OR cb.title LIKE '%arrive%' THEN general_category_id
      WHEN cb.title LIKE '%parking%' OR cb.title LIKE '%venue%' THEN venue_category_id
      WHEN cb.title LIKE '%ceremony%' THEN venue_category_id
      WHEN cb.title LIKE '%dress%' THEN dress_category_id
      WHEN cb.title LIKE '%accommodation%' THEN (SELECT id FROM faq_categories WHERE slug = 'accommodation')
      WHEN cb.title LIKE '%dietary%' THEN general_category_id
      WHEN cb.title LIKE '%photo%' THEN general_category_id
      WHEN cb.title LIKE '%reception%' THEN (SELECT id FROM faq_categories WHERE slug = 'reception')
      WHEN cb.title LIKE '%gift%' THEN (SELECT id FROM faq_categories WHERE slug = 'gifts')
      ELSE general_category_id
    END as category_id,
    cb.title as question,
    cb.content as answer,
    cb.order_index + 100 as display_order, -- Offset to avoid conflicts
    false as is_featured,
    cb.is_visible as is_active
  FROM content_blocks cb
  WHERE cb.type = 'faq' 
  AND cb.section = 'faq'
  AND NOT EXISTS (
    -- Avoid duplicates by checking similar questions
    SELECT 1 FROM faq_items fi 
    WHERE LOWER(fi.question) LIKE '%' || LOWER(SPLIT_PART(cb.title, ' ', 3)) || '%'
  );
END $$;
