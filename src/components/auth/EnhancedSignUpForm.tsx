...
export const EnhancedSignUpForm: React.FC<EnhancedSignUpFormProps> = ({ onSwitchToSignIn }) => {
  ...
  const onSignUpSubmit = async (data: SignUpFormData) => {
    try {
      ...
      const { error } = await signUp(data.email, data.password, data.firstName, data.lastName, profileData);
      if (error) {
        toast({
          title: "Sign Up Error",
