    ...
    <div className="min-h-screen" style={{ padding: '40px 20px', position: 'relative' }}>
      <div className="container" style={{ maxWidth: '1000px', margin: '0 auto', position: 'relative' }}>
        ...
        <div className="grid grid-cols-3 gap-5 mb-10">
          <div className="glass-card p-8 text-center cursor-pointer transition-all duration-300 hover:translate-y-[-5px] hover:scale-[1.02] animate-fade-up" style={{ animationDelay: '0.1s' }}>
            <div className="text-[32px] mb-3 h-10">💬</div>
            <div className="text-[36px] font-bold text-[#2d3f51] mb-1 leading-none">{messages.length}</div>
            <div className="text-sm text-[#7a736b] font-medium">Messages</div>
          </div>
          
          <div className="glass-card p-8 text-center cursor-pointer transition-all duration-300 hover:translate-y-[-5px] hover:scale-[1.02] animate-fade-up" style={{ animationDelay: '0.2s' }}>
            <div className="text-[32px] mb-3 h-10">❤️</div>
            <div className="text-[36px] font-bold text-[#2d3f51] mb-1 leading-none">
              {messages.reduce((total, msg) => total + msg.message_likes.length, 0)}
            </div>
            <div className="text-sm text-[#7a736b] font-medium">Likes</div>
          </div>
        ...
        <div className="glass-card p-8 mb-8 animate-fade-up" style={{ animationDelay: '0.5s' }}>
          <h2 className="text-xl font-semibold text-[#2d3f51] mb-5">Share a Message</h2>
          
          <textarea 
            className="w-full min-h-[120px] bg-gradient-to-br from-[rgba(230,222,214,0.3)] to-[rgba(245,237,228,0.3)] border border-[rgba(255,255,255,0.5)] rounded-[20px] p-5 text-base text-[#2d3f51] resize-y transition-all duration-300 focus:outline-none focus:border-[rgba(102,126,234,0.5)] font-[inherit]"
            style={{
          ...
          <div className="flex items-center justify-between mt-5">
            <div className="flex gap-4">
              <button className="w-9 h-9 bg-gradient-to-br from-[#e8e0d7] to-[#f5ede4] rounded-[10px] flex items-center justify-center text-[#7a736b] text-lg border-none cursor-pointer transition-all duration-300 hover:translate-y-[-2px]"
                style={{
            ...
            <button 
              ...
              className="bg-gradient-to-r from-[#667eea] to-[#764ba2] text-white border-none px-8 py-3 rounded-[25px] font-semibold text-sm cursor-pointer flex items-center gap-2 transition-all duration-300 hover:translate-y-[-2px] relative overflow-hidden disabled:opacity-50 disabled:cursor-not-allowed"
              style={{
              ...
              {messages.map((msg, index) => (
                ...
                >
                  <div className="flex items-start gap-4">
                    <div className="w-10 h-10 rounded-full bg-[#2d3f51] text-white flex items-center justify-center text-sm font-semibold flex-shrink-0">
                      {getInitials(msg.profiles)}
                    </div>
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        <h4 className="font-semibold text-[#2d3f51]">{getDisplayName(msg.profiles)}</h4>
                        <span className="text-xs text-[#7a736b]">{formatTimeAgo(msg.created_at)}</span>
                      </div>
                      <p className="text-[#7a736b] mb-3 leading-relaxed">
                        {msg.content}
                      </p>
                      <div className="flex items-center gap-4">
                        ...
                        <button className="text-[#667eea] hover:text-[#667eea]/80 transition-colors">
